...
function AdminDashboard() {
  ...
  return (
    <AdminShell activeTab={activeTab} onTabChange={openTab} onLogout={logout}>
      ...
      {activeTab === "templates" && (
        <FunnelTemplatesSection
          draft={draft}
          setDraft={setDraft}
          onEdit={() => openTab("content")}
        />
      )}
      {activeTab === "content" && (
      ...
      {(activeTab === "templates" || activeTab === "content" || activeTab === "sales") && (
...
export function ConfigSection({
  ...
  const [whatsappNumber, setWhatsappNumber] = useState(initial.sales.whatsappNumber ?? "");
  ...
  useEffect(() => {
    ...
    loadPublished().then((published) => {
      ...
      setWhatsappNumber(merged.sales.whatsappNumber ?? "");
      setWhatsappMessage(
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <div className="space-y-2 sm:col-span-2">
          <div className="mt-2 border-t border-zinc-200 pt-6">
            <h4
              className={`text-base font-black uppercase ${theme === "dark" ? "text-white" : "text-zinc-900"}`}
            >
              Configuração do WhatsApp
            </h4>
            <p className={`mt-1 text-sm ${theme === "dark" ? "text-zinc-400" : "text-zinc-500"}`}>
              Defina o destino do botão de atendimento da página de vendas.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <textarea
            rows={4}
            placeholder="Mensagem preenchida ao abrir o WhatsApp"
            value={whatsappMessage}
            onChange={(e) => {
              setWhatsappMessage(e.target.value);
              publish({ whatsappMessage: e.target.value });
            }}
            className={`w-full resize-y rounded-2xl border px-4 py-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all ${
              theme === "dark"
                ? "border-white/10 bg-black/40 text-white"
                : "border-zinc-200 bg-zinc-50 text-zinc-900 shadow-inner"
            }`}
...
function FunnelTemplatesSection({
  draft,
  setDraft,
  onEdit,
}: {
  draft: FunnelDraft;
  setDraft: (draft: FunnelDraft) => void;
  onEdit: () => void;
}) {
  ...
  const handleApplyTemplate = () => {
    ...
    toast.success(`Modelo "${selected.name}" aplicado ao rascunho.`, {
      description: "Agora você pode editar todas as etapas antes de publicar.",
    });
  };

  return (
    <section className="admin-panel overflow-hidden p-5 sm:p-7">
      <div className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/10 via-white/[0.03] to-transparent p-5 sm:p-7">
        ...
        <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-white/[0.07] bg-black/20 p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-xs font-black uppercase tracking-widest text-white transition hover:opacity-90"
            >
            ...
            <button
              type="button"
              onClick={onEdit}
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-5 py-3 text-xs font-black uppercase tracking-widest text-zinc-200 transition hover:bg-white/[0.08]"
            >
              Editar conteúdo
