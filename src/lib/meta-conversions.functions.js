...
function validateMetaInput(input) {
    if (!ALLOWED_EVENTS.has(input.eventName))
        throw new Error("Evento da Meta não permitido.");
    // Suaviza o regex para aceitar IDs gerados pelo webhook (prefixo wb_)
    if (!/^[a-zA-Z0-9_-]{3,120}$/.test(input.eventId))
    ...
    const customData = Object.fromEntries(Object.entries(input.customData ?? {})
        .slice(0, 30)
...
async function deliverMetaConversion(data, options) {
    ...
    if (!pixelId || !accessToken) {
        return { sent: false, status: "not_sent", reason: "not_configured" };
    }
    if (options.test && !testEventCode) {
        return { sent: false, status: "not_sent", reason: "test_not_configured" };
    }
    ...
    const externalId = data.sessionId
        ...
        : undefined;
    const hash = (val) => val ? createHash("sha256").update(val.toLowerCase().trim()).digest("hex") : undefined;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { sent: false, status: "error", reason: "api_error", error };
        }
        ...
        return { sent: false, status: "error", reason: "network_error", error };
    }
}
function isSameOriginSource(eventSourceUrl) {
    try {
        const request = getRequest();
        ...
        if (projectUrl && sourceOrigin.includes(projectUrl))
            return true;
        return false;
    }
    catch {
...
async function assertAdmin(userId, accessToken) {
...
export const sendMetaConversion = createServerFn({ method: "POST" })
    ...
    if (!isWebhook && !isSameOriginSource(data.eventSourceUrl)) {
        ...
        return { sent: false, status: "skipped", reason: "invalid_origin" };
    }
